...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    return programmersList[progID-1].name
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: progID != null ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmersList.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { setProgId(dev.id)}}>Feature</button>
            </div>
          )
        }
      </div>
